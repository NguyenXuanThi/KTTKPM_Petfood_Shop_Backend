...
if (enabled) {
  ...
  redis.on("connect", () => console.log("[category-service] Redis connected"));
  redis.on("error", (error) =>
    console.warn(`[category-service] Redis error: ${error.message}`),
  );
} else if (redisEnabled && !Redis) {
  console.warn("[category-service] ioredis is not installed. Redis cache disabled.");
}
...
const connectRedis = async () => {
  ...
  connectPromise = redis
    ...
    .catch((error) => {
      connectPromise = null;
      console.warn(`[category-service] Redis connection failed: ${error.message}`);
      return null;
    });
...
const safeRedis = async (operation, fallback = null) => {
  ...
  try {
    ...
    console.warn(`[category-service] Redis operation failed: ${error.message}`);
    return fallback;
  }
};
