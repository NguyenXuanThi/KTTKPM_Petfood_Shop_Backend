  ...
  io.on('connection', (socket) => {
    ...
    socket.on('joinConversation', async ({ conversationId, userId, role }) => {
      try {
        ...
        console.log(`Socket ${socket.id} joinConversation payload:`, { conversationId, userId, role });
    ...
    socket.on('sendMessage', async (payload) => {
      try {
        ...
        const res = await liveService.addMessage(conversationId, senderId, senderRole, message || '', senderName || '', senderAvatar || '', { messageType, fileUrl, metadata, productId: payload.productId || '', productData: payload.productData || {} });
        if (!res.success) {
