  ...
  io.on("connection", (socket) => {
    ...
    socket.on("joinConversation", async ({ conversationId, userId, role }) => {
      try {
        ...
        console.log(
          `[Socket joinConversation] ${role} user ${userId} joining conversation`,
          { conversationId, socketId: socket.id },
        );

        if (conversationId) {
          socket.join(conversationId);
          console.log(
            `[Socket joinConversation] Successfully joined room:`,
            conversationId,
          );
          ...
          if (msgs.success) {
            console.log(
              `[Socket joinConversation] Sending ${msgs.data.length} messages to ${userId}`,
            );
        ...
        if (isStaffRole(role)) {
          ...
          if (role === "support") {
            ...
            console.log(
              `[Socket joinConversation] Sending ${list.data?.length || 0} conversations to support ${userId}`,
            );
          } else {
            // For admin, get all conversations
            list = await liveService.getConversationsForAdmin();
            console.log(
              `[Socket joinConversation] Sending ${list.data?.length || 0} conversations to admin ${userId}`,
            );
          }
    ...
    socket.on("sendMessage", async (payload) => {
      try {
        ...
        console.log(
          `[Socket sendMessage] Received from ${senderName} (${senderRole}):`,
          {
            conversationId,
            senderId,
            messageType,
        ...
        if (!conversationId || !senderId) {
          console.warn(
            `[Socket sendMessage] Invalid payload - missing conversationId or senderId`,
        ...
        if (
          ...
          console.warn(
            `[Socket sendMessage] Product message without productData`,
          );
          socket.emit("error", {
        ...
        console.log(
          `[Socket sendMessage] Message saved. Emitting to room:`,
          conversationId,
        );
        ...
        console.log(
          `[Socket sendMessage] Message emitted to room ${conversationId}`,
        );

        // Get conversation participants to notify staff
        const participants =
          await liveService.getConversationParticipants(conversationId);
        if (participants.success) {
          ...
          if (adminConvs.success) {
            try {
              ...
              console.error(
                "[Socket sendMessage] broadcast conversationUpdated failed",
                e.message,
              );
            }
          }
          ...
          if (participants.data.supportId) {
            const supportConvs = await liveService.getConversationsForSupport(
              participants.data.supportId,
            );
            io.sockets.sockets.forEach((s) => {
              if (
                s.data &&
                s.data.role === "support" &&
                s.data.userId === participants.data.supportId &&
                supportConvs.success
              ) {
                console.log(
                  `[Socket sendMessage] Notifying support ${participants.data.supportId} about conversation update`,
                );
