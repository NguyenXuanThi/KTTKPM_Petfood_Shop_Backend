...
`;
  ...
  async analyzeIntent(userMessage) {
    ...
    if (
      message.includes("mua ngay") ||
      message.includes("đặt ngay") ||
      message.includes("mua luôn") ||
      message.includes("đặt luôn") ||
      message.includes("đặt hàng online") ||
      message.includes("ok mua") ||
      message.includes("đồng ý mua") ||
      message.includes("xác nhận")
    ) {
    ...
    if (
      message.includes("mua thêm") ||
      message.includes("thêm") ||
      message.includes("tiếp") ||
      message.includes("nữa") ||
    ...
    const hasQuantity =
      /(\d+|một|hai|ba|bốn|năm)\s*(hộp|bịch|gói|chai|lon|kg|g)/i.test(message);
    const hasProductRef =
      message.includes("đó") ||
      message.includes("này") ||
    ...
    if (
      ...
      message.includes("còn") ||
      message.includes("thức ăn") ||
      message.includes("sản phẩm") ||
      message.includes("cho chó") ||
      message.includes("cho mèo") ||
      message.includes("pate") ||
      message.includes("cát") ||
  ...
  extractOrderDetails(userMessage, context) {
    ...
    if (
      ...
      let productIndex = 0;
      if (
        message.includes("thứ 2") ||
        message.includes("thứ hai") ||
        message.includes("cái 2")
      ) {
        productIndex = 1;
      } else if (
        message.includes("thứ 3") ||
        message.includes("thứ ba") ||
        message.includes("cái 3")
      ) {
        productIndex = 2;
      ...
      if (context.products[productIndex]) {
        ...
        console.log(
          "Added to order:",
          context.products[productIndex].name,
          "x",
          quantity,
        );
      }
  ...
  async enrichContextWithProducts(userMessage) {
    try {
      ...
      // Search for products
      const searchResults = await productClient.searchProducts(
        keywords.join(" "),
        limit,
      );
