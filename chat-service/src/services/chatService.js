    ...
    try {
      ...
      if (intent.intent === "order") {
        ...
        if (orderDetails.hasOrder) {
          ...
          orderDetails.items.forEach((item) => {
            const existingItem = context.cart.find(
              (ci) => ci.product._id === item.product._id,
            );
            if (existingItem) {
        ...
        const productContext =
          await aiService.enrichContextWithProducts(userMessage);
      ...
      console.log(
        "Saving context to conversation:",
      ...
      console.log(
        "Conversation saved. Context after save:",
        JSON.stringify(conversation.context, null, 2),
      );
