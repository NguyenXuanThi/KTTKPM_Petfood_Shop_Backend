...
if (enabled) {
  ...
  redis.on("connect", () => console.log("[coupon-service] Redis connected"));
  redis.on("error", (error) =>
    console.warn(`[coupon-service] Redis error: ${error.message}`),
  );
} else if (redisEnabled && !Redis) {
  console.warn("[coupon-service] ioredis is not installed. Redis cache disabled.");
}
...
const connectRedis = async () => {
  ...
  connectPromise = redis
    ...
    .catch((error) => {
      connectPromise = null;
      console.warn(`[coupon-service] Redis connection failed: ${error.message}`);
      return null;
    });
...
const safeRedis = async (operation, fallback = null) => {
  ...
  try {
    ...
    console.warn(`[coupon-service] Redis operation failed: ${error.message}`);
    return fallback;
  }
};
